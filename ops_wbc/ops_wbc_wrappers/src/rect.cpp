/************************************************************************
	...
	> Created Time: Wed 17 Jan 2018 04:08:01 PM PST
 ************************************************************************/

#include <ros/ros.h>
#include "ops_wbc_wrappers/rect.hpp"
...
bool Rect::pixelIsNot(uchar* prsc, int channels, int ignored_value)
{
    if (channels == 1)
    {
        if (*prsc != ignored_value)
        {
            return true;
        }
        else 
    ...
    {
        if (prsc[0] != ignored_value || prsc[1] != ignored_value || prsc[2] != ignored_value)
        {
            return true;
        }
        else 
    ...
    {
        if(prsc[0] != ignored_value
                || prsc[1] != ignored_value
                || prsc[2] != ignored_value
                || prsc[3] != ignored_value)
        {
            return true;
        }
        else
...
int Rect::getUpperY(cv::Mat& src, uchar ignored_value)
{
    ...
    {
        uchar* prsc = src.ptr<uchar>(y);

        for (int x = 0; x < width; x += channels)
        {
            if (pixelIsNot(&prsc[x], channels, ignored_value))
            {
                if(y > 0)
                {
                    return y - 1;
