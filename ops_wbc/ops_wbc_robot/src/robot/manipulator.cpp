...
void Manipulator::computeForwardKinematics()
{
    ...
    // The distance between the i-th link and the end-effector w.r.t base 
    if (T_abs_.size() != C_abs_.size())
    {
        ...
        throw ops_wbc_utils::Exception("Manipulator::computeForwardKinematics", msg.str());
    }
    ...
    else if(Pin_.size() == 0)
    {
        std::stringstream msg;
        msg << "Pin_.size() == 0";
        throw ops_wbc_utils::Exception("Manipulator::computeTabs", msg.str());
    }
...
void Manipulator::computeCabs()
{
    if(C_abs_.size() != T_abs_.size())
    {
        ...
        throw ops_wbc_utils::Exception("Manipulator::computeCabs", msg.str());
    }
    ...
    for (uint32_t i = 0; i < T_abs_.size(); i++)
    {
        ...
        Tic.block(0, 3, 3, 1) = link_[i]->C;
        C_abs_[i] = T_abs_[i] * Tic;
    }
}
...
void Manipulator::computeJacobian(int32_t idx, Eigen::MatrixXd& J)
{
    ...
    // usually not required for the end-effector 
    for (uint32_t i = 0; i <= idx; i++)
    {
        if (link_[i]->ep) // prismatic joint 
        {
    ...
    if (idx == dof_) // Required to consider the offset of end-effector
    {
        Eigen::MatrixXd J_Pne = Eigen::Matrix4d::Identity(6 ,6);    
        Eigen::Vector3d Pne;
...
void Manipulator::computeVelocity()
{
  if(updated_joint_)
  {
    pseudo_differentiator_->apply(q_);
    pseudo_differentiator_->copyDerivativeValueTo(dq_);
  }
}
