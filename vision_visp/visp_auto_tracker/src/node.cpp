        ...
        void Node::spin(){
                ...
                while(ros::ok()){
                  double t = vpTime::measureTimeMs();
                        ...
                        ps.header.frame_id = "QR_object";
                        transform_.setOrigin(tf::Vector3(ps.pose.position.x, ps.pose.position.y, ps.pose.position.z) );
                        transform_.setRotation(tf::Quaternion(ps.pose.orientation.x, ps.pose.orientation.y, ps.pose.orientation.z, ps.pose.orientation.w));
                        br_.sendTransform(tf::StampedTransform(transform_, ros::Time::now(), "camera_rgb_optical_frame", "QR_object"));

                        /*
                        //simply to test the tf publisher
                        transform_.setOrigin(tf::Vector3(-0.280, 0, -0.025));
                        tf::Quaternion q;
                        q.setRPY(3.14159265358979323846 / 2, 0, 3.14159265358979323846 / 2);
                        transform_.setRotation(q);
                        br_.sendTransform(tf::StampedTransform(transform_, ros::Time::now(), "QR_object", "start_frame"));
                        */
                        //transform_.setOrigin(tf::Vector3(0.102, -0.715, 1.037));
                        //q.setRPY(3.14159265358979323846, 0, 0);
                        //transform_.setRotation(q);
                        //br_.sendTransform(tf::StampedTransform(transform_, ros::Time::now(), "left_wam/base_link", "camera_rgb_optical_frame"));



                        // Publish resulting pose and the tf transformation.
