...
namespace function_approximator
{
    ...
    class UnifiedModel : public Parameterizable
    {
        ...
        public:
             ...
             * \param[in] lines_pivot_at_max_activation Whether line models should pivot at x=0 (false), or at the center of the kernel (x=x_c)
             */
            UnifiedModel(const std::vector<Eigen::VectorXd>& centers, // n_centers X n_dims
        ...
        private:
            std::vector<Eigen::VectorXd> centers_; // n_bfs X n_dims
            std::vector<Eigen::MatrixXd> covars_;  // n_bfs X n_dims X n_dims
            std::vector<Eigen::VectorXd> slopes_;  // n_bfs X n_dims
            std::vector<double> offsets_;          // n_bfs X 1
            std::vector<double> priors_;           // n_bfs X 1
...
BOOST_CLASS_EXPORT_KEY2(function_approximator::UnifiedModel, "UnifiedModel")

/** Don't add version information to archives. */
BOOST_CLASS_IMPLEMENTATION(function_approximator::UnifiedModel,boost::serialization::object_serializable);
