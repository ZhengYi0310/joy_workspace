/*************************************************************************
	> File Name: UnifiedModel.cpp
	> Author: Yi Zheng 
	> Mail: hczhengcq@gmail.com
	> Created Time: Thu 17 Aug 2017 12:00:27 PM PDT
 ************************************************************************/
...
BOOST_CLASS_EXPORT_IMPLEMENT(function_approximator::UnifiedModel);

#include <function_approximator/FunctionApproximator.hpp>
#include <function_approximator/BasisFunction.hpp>
...
namespace function_approximator
{
    UnifiedModel::UnifiedModel(const Eigen::MatrixXd& centers, const Eigen::MatrixXd& widths, const Eigen::VectorXd& weights,bool normalized_basis_functions, bool lines_pivot_at_max_activation)
    {
        ...
        for (int i = 0; i < n_basis_functions; i++)
        {
            ...
            covars_[i] = (widths.rows(i).asDiagonal()).array().square();
            offsets_[i] = weights[i];
            
            slopes_[i] = Eigen::VectorXd::Zero(n_dims);
            priors_[i] = 1.0;
        }
    ...
    UnifiedModel::UnifiedModel(const std::vector<Eigen::VectorXd>& centers, // n_centers X n_dims
    ...
    UnifiedModel::initializeAllValuesVectorSize()
    {
