/*************************************************************************
	> File Name: FunctionApproximator.cpp
	> Author: Yi Zheng 
	> Mail: hczhengcq@gmail.com
	> Created Time: Wed 16 Aug 2017 07:47:10 PM PDT
 ************************************************************************/
#include <function_approximator/FunctionApproximator.hpp>
#include <function_approximator/ModelParameters.hpp>
#include <function_approximator/MetaParameters.hpp>
#include <function_approximator/UnifiedModel.hpp>

#include <function_approximator_io/EigenFileIO.hpp>
#include <function_approximator_io/EigenBoostSerialization.hpp>
#include <function_approximator_io/BoostSerializationToString.hpp>
...
namespace function_approximator
{
    ...
    void FunctionApproximator::getParameterVectorSelectedMinMax(Eigen::VectorXd& min, Eigen::VectorXd& max) const
    {
        if (model_parameters_== NULL)
        {
            std::cerr << __FILE__ << ":" << __LINE__ << ": Warning: Trying to access model parameters of the function approximator, but it has not been trained yet. Returning empty parameter vector." << endl;
            min.resize(0);
            max.resize(0);
            return;
        }
    ...
    bool FunctionApproximator::checkModelParametersInitialized(void) const
    {
        if (model_parameters_==NULL)
        {
            std::cerr << "Warning: Trying to access model parameters of the function approximator, but it has not been trained yet. Returning empty parameter vector." << endl;
            return false;
        }
        return true;
    }
    ...
    /******************************************************************************/
    void FunctionApproximator::setSelectedParameters(const set<string>& selected_parameters_labels)
    {
        if (checkModelParametersInitialized())
        {
            model_parameters_->setSelectedParameters(selected_parameters_labels);
        }
    }
    /******************************************************************************/
    ...
    bool FunctionApproximator::saveGridData(const VectorXd& min, const VectorXd& max, const VectorXi& n_samples_per_dim, string save_directory, bool overwrite) const
    {
        ...
        {
            ROS_WARN("No save directory is provided, return...");
            return true;
        }

        if (model_parameters_ == NULL)
        {
            ROS_ERROR("Model parameters pointer is NULL, return...");
            return false;
        }
        UnifiedModel* mp_unified = model_parameters_->toUnifiedModel();
        if (mp_unified == NULL)
        {
            ROS_ERROR("Unified model pointer is NULL, return...");
            return false;
        }
    ...
    void FunctionApproximator::train(const Eigen::MatrixXd& inputs, const Eigen::MatrixXd& targets, std::string save_directory, bool overwrite)
    {
        ...
        if (!isTrained())
        {
            ROS_WARN("Model parameters pointer is still NULL after training, return...");
            return;
        }
